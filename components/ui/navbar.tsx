...
function MobileMenu({ menuOpen, setMenuOpen }: any) {
  return (
    <div
...
function MenuBar({ menuOpen, setMenuOpen }: any) {
  function notProtected(item: any) {
    if (item.protected && !isLoaded) {
      return false;
    }
    if (item.protected && isLoaded && !userId) {
      return false;
    }
    return true;
  }
  ...
  const navLinks = navItems.filter(notProtected).map((item) => (
    <Menu
      key={item.id}
      as="div"
    ...
    <>
      <div className="justify-around xl:container xl:mx-auto xl:flex xl:max-w-7xl ">
        <div className="flex items-center justify-between p-2 font-display sm:text-xs md:text-sm xl:container xl:text-xl">
          <div className="items-left" onClick={() => setMenuOpen(false)}>
